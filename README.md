...
You give it a style description in plain english. Something like "black and white macro botanical photography, high contrast, minimal composition, technically precise." It pulls your Instagram posts, runs each image through a vision LLM, and the LLM decides: does this fit the vibe or not.
...
**The AI Worker** (`/ai_worker`) — Python, FastAPI, Instructor. This is the muscle. Stateless. It gets an image over HTTP, encodes it to base64, asks the vision model what it sees, and enforces the response into a strict Pydantic schema via `instructor`. It also handles Instagram downloads via `instaloader`.
...
Instagram requires a session to access your posts. Set your credentials in `ai_worker/.env` and run this once:

```bash
make instagram-auth
```

That logs in, saves a session file to `~/.config/instaloader/session-<username>`, and from that point forward the worker loads the session at startup, we dont need no passwords flying around on every request, no "unusual login" alerts from Instagram. If the session ever expires, just run it again.
